...
import Link from 'next/link'; // react-router-dom yerine Next.js Link entegrasyonu
import { Compass, MapPin, Route } from 'lucide-react';
import { getToursFromWordPress } from '@/services/wp-api'; // alias path düzeltmesi
import { TourListing } from '@/types';
import { SEO } from '@/components/SEO';

// Default Leaflet marker hatası için Next.js uyumlu fix
...
const TourMarker: React.FC<{ tour: TourListing }> = ({ tour }) => {
  if (!tour.coordinates || !customIcon) return null;
  return (
    <Marker 
      ...
      <Popup closeButton={false} className="custom-popup">
        <div className="w-56 flex flex-col p-0 pb-1 m-[-14px]">
          ...
          <div className="px-3">
            ...
            {/* react-router Link yapısı Next href'e dönüştürüldü */}
            <Link 
              href={`/tour/${tour.id}`}
...
export default function MapPage() {
  ...
  const userIcon = React.useMemo(() => typeof window !== 'undefined' ? new L.Icon({
        ...
        <div className="flex flex-col gap-2">
          <button 
            ...
            className={`w-full flex items-center justify-center gap-2 py-2.5 rounded-xl font-bold transition-all ${
              showRoute ? 'bg-orange-100 text-orange-600 border border-orange-200' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
            }`}
          >
